...
def generate_sparse(path: str, size: int) -> None:
    ...
    print(f"[sparse] Done — file created instantly, real disk usage: ~0 bytes")
...
def generate_fill(path: str, size: int) -> None:
    ...
    print(f"[fill] Writing {size / 1e9:.1f} GB to {path} in {CHUNK_SIZE // 1024} KB chunks")
...
def generate_text(path: str, size: int) -> None:
    ...
    print(f"\n[text] Done in {time.monotonic() - start_time:.1f}s — {line_num:,} lines written")
...
def main() -> None:
    ...
    if strategy not in generators:
        print(f"Unknown strategy '{strategy}'. Choose: sparse, fill, text", file=sys.stderr)
        sys.exit(1)
